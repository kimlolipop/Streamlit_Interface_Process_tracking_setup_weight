...
def _resize_img(img: Image, max_height: int=700, max_width: int=700):
    ...
    if img.height > max_height:
        ratio1 = max_height / img.height
        img = img.resize((int(img.width * ratio1), int(img.height * ratio1)))
        
    if img.width > max_width:     
        ratio2 = max_width / img.width
        img = img.resize((int(img.width * ratio2), int(img.height * ratio2)))
    return img, ratio1, ratio2
...
def st_cropper(img: Image, realtime_update: bool=True, box_color: str='blue', aspect_ratio: tuple=None,
    ...
    img, ratio1, ratio2 = _resize_img(img)
...
#         return [{'left':rect['left'], 'top':rect['top']}]
        return cropped_img, [(rect['left']/ratio1)/ratio2, (rect['top']/ratio1)/ratio2, ((rect['width'] + rect['left'])/ratio1)/ratio2, ((rect['height'] + rect['top'])/ratio1)/ratio2], ratio1, ratio2
